...
// ============================================================
const { SECTION_SLOTS, snackStyleFor } = require('./generator');
const { AI_CATEGORIES } = require('./aiMenu');
const { cuisineForDate } = require('./nationalDay');

const NO_REPEAT_DAYS = 28;
const MEAT = ['CHICKEN', 'BEEF', 'LAMB', 'FISH', 'TURKEY'];
...
function checkDraftRules(picks, attrOf) {
  ...
  for (const section of ['DAYCARE', 'KG_LP', 'MS_UP', 'STAFF']) {
    dates.forEach((date, dayIndex) => {
      ...
      if (section !== 'STAFF') {
        // Pastry / Cold Kitchen rotation for both snacks (lib/generator.js SNACK_STYLE_BY_PATTERN).
        // KG-LP's snacks are copies of Daycare's, so they are checked (and noted) on Daycare only.
        for (const [category, label] of [['AM_SNACK', 'AM Snack'], ['PM_SNACK', 'PM Snack']]) {
          if (section === 'KG_LP') {
            const mine = dayPicks('KG_LP', date, category).map(keyOf).sort().join();
            const theirs = dayPicks('DAYCARE', date, category).map(keyOf).sort().join();
            if (mine !== theirs) note(section, date, category, `should be identical to Daycare's ${label} (shared)`);
            continue;
          }
          const want = snackStyleFor(dayIndex + 1, category, section);
          for (const { a } of attrs(category)) {
            if (a.am_snack_style && a.am_snack_style !== want) {
              note(section, date, category, `${label} should be ${STYLE_LABEL[want]} today (Pastry / Cold Kitchen rotation); "${a.name}" is ${STYLE_LABEL[a.am_snack_style]}`);
            }
          }
        }
      }
      ...
      if (section === 'STAFF') {
        const MEAT_FREE = ['VEGAN', 'VEGETARIAN'];
        const bf = attrs('STAFF_BREAKFAST');
        if (bf.length && !bf.some((x) => MEAT_FREE.includes(x.a.protein_code))) note(section, date, 'STAFF_BREAKFAST', 'Staff Breakfast has no vegan or vegetarian dish');
        // 3 Pastry + 3 Cold Kitchen across the six, the shared school AM Snacks included.
        if (bf.length === 6) {
          const pastry = bf.filter((x) => x.a.am_snack_style === 'PASTRY').length;
          const cold = bf.filter((x) => x.a.am_snack_style === 'COLD_KITCHEN').length;
          if (pastry !== 3 || cold !== 3) note(section, date, 'STAFF_BREAKFAST', `Staff Breakfast should be 3 Pastry + 3 Cold Kitchen (now: ${pastry} Pastry, ${cold} Cold Kitchen${6 - pastry - cold ? `, ${6 - pastry - cold} without a style` : ''})`);
        }
        ...
        // Staff Main: its own pair (the non-shared picks) is 1 vegan + 1 vegetarian with different starches.
        const own = attrs('STAFF_MAIN').filter((x) => !x.p.source_section_code);
        if (own.length) {
          if (!own.some((x) => x.a.protein_code === 'VEGAN')) note(section, date, 'STAFF_MAIN', "Staff Main's own dishes have no vegan dish");
          if (!own.some((x) => x.a.protein_code === 'VEGETARIAN')) note(section, date, 'STAFF_MAIN', "Staff Main's own dishes have no vegetarian dish");
          const carbs = own.map((x) => x.a.carb_type).filter(Boolean);
          if (new Set(carbs).size < carbs.length) note(section, date, 'STAFF_MAIN', `Staff Main's vegan and vegetarian dishes have the same starch type (${carbs[0]})`);
        }
        const box = attrs('STAFF_LUNCHBOX').map((x) => x.a.protein_code);
        if (box.length === 2 && !(box.some((c) => MEAT_FREE.includes(c)) && box.some((c) => MEAT.includes(c)))) {
          note(section, date, 'STAFF_LUNCHBOX', 'Lunch Box should be one meat or fish dish and one vegan or vegetarian dish');
        }
        // The shared dishes Staff Main must carry.
        const staffMain = new Set(dayPicks('STAFF', date, 'STAFF_MAIN').map(keyOf));
        for (const [src, cat, label] of [['KG_LP', 'LUNCH_MAIN', 'Lunch Main'], ['KG_LP', 'LUNCH_STARCH', 'Lunch Starch'], ['MS_UP', 'LUNCH_VEGETABLE', 'Lunch Vegetable']]) {
          for (const p of dayPicks(src, date, cat)) {
            if (keyOf(p) && !staffMain.has(keyOf(p))) note(section, date, 'STAFF_MAIN', `Staff Main is missing ${SECTION_LABEL[src]}'s ${label} "${attrOf(p)?.name}"`);
          }
        }
        const staffBf = new Set(dayPicks('STAFF', date, 'STAFF_BREAKFAST').map(keyOf));
        // KG-LP's AM Snack is Daycare's (shared), so Daycare + MS-UP cover both shared dishes.
        for (const src of ['DAYCARE', 'MS_UP']) {
    ...
    // National Day: on a Tuesday every AI category serves that day's cuisine. Shared copies are
    // checked where they come from.
    for (const date of dates) {
      const cuisine = cuisineForDate(date);
      if (!cuisine) continue;
      for (const p of dayPicks(section, date)) {
        if (p.source_section_code || !(AI_CATEGORIES[section] || []).includes(p.category_code)) continue;
        const a = attrOf(p);
        if (a && a.cuisine !== cuisine) note(section, date, p.category_code, `National Day (${cuisine}): "${a.name}" isn't a ${cuisine} dish`);
      }
    }

    // Daycare's Lunch Main: never the same protein two school days running (within the run; the
    // day before the run was handled when the menu was generated).
    if (section === 'DAYCARE') {
      let prev = null;
      for (const date of dates) {
        const ps = dayPicks(section, date, 'LUNCH_MAIN').map((p) => attrOf(p)).filter(Boolean);
        const proteins = new Set(ps.map((a) => a.protein_code).filter(Boolean));
        for (const a of ps) {
          if (prev && a.protein_code && prev.has(a.protein_code)) note(section, date, 'LUNCH_MAIN', `"${a.name}" is ${a.protein_code.toLowerCase()} again, like the previous school day`);
        }
        prev = proteins;
      }
    }
