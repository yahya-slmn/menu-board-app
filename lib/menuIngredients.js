...
// Inserts one new "Ingredients" column immediately after the dish-name column (rather than
// appending past every other column -- age-group GM/ML/pax columns for School, department
// columns for Staff -- which otherwise leaves Ingredients stranded several empty-looking columns
// away from the dish names it belongs to; see dishColumnBySheet, from parseWorkbookDishes).
// Inserting there (not before or elsewhere in the sheet) is specifically what keeps this safe:
...
async function appendIngredientsColumn(workbook, rowsWithIngredients, dishColumnBySheet = {}) {
  ...
  for (const [sheetName, rows] of bySheet) {
    ...
    const col = dishCol ? dishCol + 1 : sheet.columnCount + 1;
    // Only needed when something already occupies `col` (age-group/department columns, etc.) --
    // if the dish column happens to already be the sheet's last one, inserting would just shift
    // in an empty column for nothing; writing straight into `col` is equivalent and simpler.
    // Computed/applied before `maxCol` below, since a real insert shifts the sheet's own column
    // count by one and header-row re-detection needs to scan across the post-insert width.
    if (col <= sheet.columnCount) sheet.spliceColumns(col, 0, []);
