...
function scanRowMarkers(row, maxCol) {
  ...
  for (let c = 1; c <= maxCol; c++) {
    ...
    // "Quantity" -- buildSchoolSheet's own header text for its one combined-quantity column
    // (was "GM/ML" before that layout's per-age-group headcount columns were removed; renamed
    // there for clarity, so the marker here has to match).
