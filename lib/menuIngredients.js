...
function scanRowMarkers(row, maxCol) {
  ...
  return { dateCol, dateVal, weekdayCol, weekdayVal, rcCols, quantityCol, weightUnitCol };
}
...
//
// If neither "Quantity" nor "Weight/Unit" survives and there aren't 2 "RC" cells either, layout
    ...
    for (let hi = 0; hi < headers.length; hi++) {
      ...
      if (layout === 'CEO') {
        ...
        if (layout === 'AMBIGUOUS') {
          ...
          warnings.push(
            `${sheet.name}, ${h.weekday} ${h.date}: this day's RC/Quantity/Weight-Unit columns were ` +
