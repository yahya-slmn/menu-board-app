...
function parseWorkbookDishes(workbook, schoolCategoryNames = []) {
  const schoolVocab = new Set(schoolCategoryNames.filter(Boolean));
  const rows = [];
  const warnings = [];

  workbook.eachSheet(sheet => {
    if (sheet.name === '_Lists') return;
    if (HIDDEN_SHEET_STATES.has(sheet.state)) return;

    const maxCol = Math.max(sheet.columnCount || 0, 10);
    const headers = [];
    sheet.eachRow((row, rowNumber) => {
      const h = detectHeader(row, maxCol);
      if (h) headers.push({ rowNumber, ...h });
    });
    if (headers.length === 0) return; // not a menu sheet this parser recognizes -- skip quietly
...
function appendIngredientsColumn(workbook, rowsWithIngredients) {
  ...
  for (const [sheetName, rows] of bySheet) {
    ...
    const headerRows = [];
    sheet.eachRow((row, rowNumber) => { if (detectHeader(row, maxCol)) headerRows.push(rowNumber); });
