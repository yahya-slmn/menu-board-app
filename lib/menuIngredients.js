...
// A header's date cell: "29-09-2026", or with a hand-typed note after it ("29-09-2026 Arminian Day" --
// without this a whole day's rows were read as part of the day before). The date is the first 10 chars.
...
// 'Fruit Basket' / per-drink labels are older Staff layouts (Fruit Basket left Staff lunch
// 2026-09-24; the three drinks share one 'Beverages' label since then) -- kept so older files parse.
const STAFF_ITEM_VOCAB = new Set(['Main Dish', 'Juice', 'Appetizer', 'Salad', 'Sweets', 'Bread', 'Beverages', 'Fruit Basket', 'Water', 'Soft Drink', 'Fresh Juice']);
const CEO_PERSON_SET = new Set(CEO_PERSONS);
// School vs Staff by vocabulary (no Quantity / Weight-Unit marker): the fit is "clear" when the winner
// matches at least this share of its item-type / category cells and beats the other by this margin.
const CLEAR_FIT_MIN = 0.5;
const CLEAR_FIT_MARGIN = 0.3;
// 'Raw Veg' / 'Fruits' are the pre-2026-09-25 CEO rows (kept so older files parse); 'Snack' replaced
// 'Fruits', and 'Protein' is a second label on the Lunch Main Dish's own merged cells (see extractRowDish).
const CEO_ITEM_VOCAB = new Set(['Main Dish', 'Protein', 'Raw Veg', 'Juice', 'Yogurt', 'Salad', 'Fruits', 'Snack', 'Bread']);
...
// restructureAndAppendIngredients are otherwise tight synchronous loops over every row of every sheet,
...
function scanRowMarkers(row, maxCol) {
  let dateCol = null, dateVal = null, weekdayCol = null, weekdayVal = null, quantityCol = null, weightUnitCol = null;
  const rcCols = [];
  const personCols = [];
  for (let c = 1; c <= maxCol; c++) {
    const v = cellText(row, c);
    if (!v) continue;
    if (dateCol === null && DATE_RE.test(v)) { dateCol = c; dateVal = v.slice(0, 10); continue; }
    // A weekday cell may carry a National Day label after a "·" ("TUESDAY · ARMENIAN DAY", AI Menu
    // Generator exports): the weekday is the text before it.
    const weekdayPart = v.split('\u00B7')[0].trim();
    if (weekdayCol === null && WEEKDAY_NAMES.has(weekdayPart.toUpperCase())) { weekdayCol = c; weekdayVal = weekdayPart; continue; }
    if (v === 'RC') { rcCols.push(c); continue; }
    // CEO's own header names its two person columns (lib/menuLayout.js) -- the CEO marker that
    // survives once exports stop carrying RC columns.
    if (CEO_PERSON_SET.has(v)) { personCols.push(c); continue; }
    // "Quantity" / "Weight/Unit" -- the School / Staff quantity-column headers of files exported
    // before 2026-09-24 (current exports have no RC or quantity columns; kept so older files still
    // read exactly as before).
    if (v === 'Quantity') { quantityCol = c; continue; }
    if (v === 'Weight/Unit') { weightUnitCol = c; continue; }
  }
  return { dateCol, dateVal, weekdayCol, weekdayVal, rcCols, personCols, quantityCol, weightUnitCol };
}
...
// something she'd realistically delete) -- RC/Quantity/Weight-Unit are refinements on top, since
// she does delete those:
//
//   CEO    (buildCeoSheet):    date + weekday + 2 "RC" cells (one per person column), OR the two
//                               CEO person names themselves (exports without RC columns; with these
//                               the date may be missing -- the hand-made week_04 reference has none)
//   School (buildSchoolSheet): date + weekday + a "Quantity" cell
//   Staff  (buildStaffSheet):  date + weekday + a "Weight/Unit" cell
//
// The RC / Quantity / Weight-Unit markers are checked FIRST, so files exported before those
// columns were dropped read exactly as they always did. Without them (current exports, or a file
// she trimmed), layout comes back 'AMBIGUOUS' -- parseWorkbookDishes resolves it by which of
// School's or Staff's vocabulary the block's data matches, warning only when that isn't clear.
function detectHeader(row, maxCol) {
  const m = scanRowMarkers(row, maxCol);
  if (!m.weekdayVal) return null;
  // A CEO day header may carry no date (the hand-made "September week_04" reference layout): the
  // weekday plus both person names is unambiguous enough. Every other layout still needs its date.
  if (!m.dateVal && m.personCols.length < 2) return null;
  const base = { date: m.dateVal || '', weekday: m.weekdayVal, ...m };
  if (m.rcCols.length >= 2) return { ...base, layout: 'CEO' };
  if (m.personCols.length >= 2) return { ...base, layout: 'CEO' };
  if (m.quantityCol) return { ...base, layout: 'SCHOOL' };
...
// marker -- RC, Quantity, Weight/Unit, CEO's own 2 dish columns; NOT date/weekday, which get
...
// Share of rows (where both are non-blank) in which columns a and b hold the same text.
function sameValueShare(itemRows, a, b) {
  let both = 0, same = 0;
  for (const row of itemRows) {
    const x = cellText(row, a), y = cellText(row, b);
    if (!x || !y) continue;
    both++;
    if (x === y) same++;
  }
  return both >= 3 ? same / both : 0;
}
...
// The column whose non-blank item-row values are mostly meal-period labels (PERIOD_VOCAB), or null.
function bestPeriodColumn(candidateCols, itemRows) {
  let best = null, bestShare = 0.5;
  for (const c of candidateCols) {
    const values = itemRows.map(r => cellText(r, c)).filter(Boolean);
    if (!values.length) continue;
    const share = values.filter(v => PERIOD_VOCAB.has(v)).length / values.length;
...
function extractRowDish(block, row) {
  ...
  if (layout === 'CEO') {
    // The Protein row's dish cells are the Main Dish's own, merged down (cellText reads the master's
    // value on every row of a merge): the same dish, never a second one.
    if (category === CEO_PROTEIN_LABEL) return null;
...
// `rowNumber` is the stable key restructureAndAppendIngredients later uses to write each row's (possibly
...
async function parseWorkbookDishes(workbook, schoolCategoryNames = []) {
  ...
  const rows = [];
  const warnings = [];
  // Rightmost dish column actually detected per sheet -- restructureAndAppendIngredients deletes
  // every column to its right (Category/RC/Item Name/Quantity is all buildSchoolSheet/
  // buildStaffSheet/buildCeoSheet write now) and inserts "Ingredients"/"Allergens" immediately
  // after it. Tracks the MAX across every header block in a sheet, in case a day's detected dish
  // column ever genuinely differs from another day's in the same sheet -- inserting after the
  // rightmost one can never truncate a narrower day's own dish column.
  ...
  for (const sheet of workbook.worksheets) {
    ...
    for (let hi = 0; hi < headers.length; hi++) {
      ...
      // Only RC/Quantity/Weight-Unit/CEO's-own-dish-columns are excluded here -- NOT date/weekday.
      ...
      const markerCols = new Set([h.quantityCol, h.weightUnitCol, ...h.rcCols].filter(Boolean));
      ...
      if (layout === 'CEO') {
        // Old files: each person's dish is right of their RC column. Current files have no RC
        // columns, and the dish sits directly under the person's name.
        dishCols = h.rcCols.length >= 2 ? h.rcCols.map(c => c + 1) : h.personCols.slice();
        ...
        if (layout === 'AMBIGUOUS') {
          ...
          // never decided here: it is recognised above by its 2 RC cells or its 2 person-name
          // header cells, which locate its 2 dish columns. A CEO sheet that has lost BOTH would
          // fall back to one dish column here and can drop the Bread row (only one person gets
          // Bread) -- the warning below says so.
          layout = staffFit > schoolFit ? 'STAFF' : 'SCHOOL';
          // A clear vocabulary fit is simply how a current export (no RC / Quantity / Weight-Unit
          // columns) is read -- only an unclear one is worth telling her about.
          const win = Math.max(staffFit, schoolFit), lose = Math.min(staffFit, schoolFit);
          if (!(win >= CLEAR_FIT_MIN && win - lose >= CLEAR_FIT_MARGIN)) warnings.push(
            `${sheet.name}, ${h.weekday} ${h.date}: couldn't clearly tell whether this day is a School or ` +
            `a Staff menu from its categories (read as ${layout === 'STAFF' ? 'Staff' : 'School'}) -- worth a ` +
            `quick check of this day's rows. If this sheet is actually CEO's, keep its "${CEO_PERSONS.join('" and "')}" ` +
            `header cells and re-upload.`
          );
        }
        ...
        // A CEO sheet that lost both its RC and person-name header cells still has CEO's own
        // signature: a SECOND dish column repeating the first on almost every row (both persons
        // get the same dish except Bread). Read it as CEO with both columns rather than silently
        // dropping the Bread row, and say so.
        const twin = picked.dishCol && candidateCols.find((c) => c !== picked.dishCol && c !== categoryCol && sameValueShare(itemRows, picked.dishCol, c) >= 0.6);
        if (twin) {
          layout = 'CEO';
          dishCols = [picked.dishCol, twin].sort((a, b) => a - b);
          categoryCol = pickCategoryAndDishColumns(candidateCols.filter((c) => !dishCols.includes(c)), itemRows, CEO_ITEM_VOCAB).categoryCol;
          warnings.push(
            `${sheet.name}, ${h.weekday} ${h.date}: read as a CEO menu from its two matching dish columns -- its ` +
            `"${CEO_PERSONS.join('" and "')}" header cells were missing. Worth a quick check of this day's rows.`
          );
        }
      }
      ...
      // The meal-period column (Breakfast / Lunch / Lunch Box, merged down column 1 in every layout):
      // Staff and CEO reuse item labels across periods ("Main Dish" is Breakfast AND Lunch), so the Dish
      // Catalog import (lib/catalogImport.js) needs the period to tell them apart. '' when not found.
      const periodCol = bestPeriodColumn(range(1, maxCol).filter(c => c !== categoryCol && !dishCols.includes(c)), itemRows);
      ...
      itemRows.forEach((row, idx) => {
        ...
        rows.push({
          ...
          layout, period: periodCol ? cellText(row, periodCol) : '',
        });
      });
...
// Layout-agnostic fallback input, for main.js's AI-assisted dish extraction (see
// extractDishesWithAI/parse-and-generate-recipes) when parseWorkbookDishes' own strict marker
// matching above finds nothing -- a real school-provided menu file (different column order,
// wording, extra per-group quantity columns, no RC/Quantity/Weight-Unit markers at all) never
// satisfies detectHeader, but an AI reading the raw grid can still tell a dish name from a day
// header/category label/quantity value. Deliberately dumb and pure (no AI/Supabase here, matching
// this file's own "file in, file out" contract stated at the top) -- just flattens one sheet into
// non-empty rows of non-empty cell text, left-to-right, dropping blanks (they carry no signal and
// only cost tokens) and skipping wholly-blank rows outright (a spacer row, or a row she cleared).
// Reuses cellText() so a merged category/period cell reads its real value on every row it spans,
// same as parseWorkbookDishes' own item-row scan does.
function flattenSheetForAI(sheet) {
  const maxCol = Math.max(sheet.columnCount || 0, 10);
  const out = [];
  for (let r = 1; r <= sheet.rowCount; r++) {
    ...
    if (cells.length > 0) out.push({ rowNumber: r, cells });
  }
  return out;
}

// Converts every formula cell in the WHOLE workbook to its plain cached computed value -- same
// effect as Excel's own "Paste Special -> Values Only", automated. Deterministic ExcelJS
// cell-by-cell work, no AI/model involved, same reasoning as why column-role detection below
// didn't need one either. Unconditional across every sheet/cell regardless of column position
// (not scoped to "right of dish name" -- see restructureAndAppendIngredients' own comment on
// why this runs first, before ANY column is touched): a formula cell's `.value` is an object
// carrying both the formula text and its last-computed `.result` (ExcelJS's own shape, already
// relied on by cellText's `raw.result` branch above) -- replacing the cell's value with just that
// result is the whole operation. `.result` can itself be a rich value (a number, string, Date,
// boolean, or an `{error:...}` object for a cell that was showing e.g. #REF!) -- passed through
// as-is either way, since all of those are already valid static ExcelJS cell values.
function stripFormulasToValues(workbook) {
  for (const sheet of workbook.worksheets) {
...
// Rebuilds each sheet down to just what the ingredients review/export actually needs: (1) every
// formula in the workbook flattened to a static value first (stripFormulasToValues, above --
// done before any column is touched so no formula text can end up referencing a column that's
// about to move or disappear); (2) every column to the RIGHT of the dish-name column deleted
// outright (whatever a given layout/upload happens to have there -- typically nothing at all
// now that buildSchoolSheet/buildStaffSheet/buildCeoSheet's own exports stop right at the
// Quantity/Weight-Unit column, but an older-format upload could still have RC codes or
// age-group/department columns out there); (3) "Ingredients" then "Allergens" written
// immediately after dish name, the new rightmost columns. Everything AT OR LEFT of dish name --
// date, weekday, category/item-type, and for CEO its own earlier RC|Name pair -- is never
// touched structurally (no column deleted, inserted, or moved there); "untouched" here means
// column position/content is preserved, not that a formula cell there is exempted from step 1 --
// a formula's cached result already IS what was visibly displayed, so flattening it changes
// nothing she can see, only removes the (now would-be-stale, since surrounding columns are about
// to change) live recalculation.
//
// This REPLACES the previous append-only behavior outright (no toggle) -- this export's whole
// purpose is a clean dish+ingredients+allergens sheet for kitchen review, and the columns being
// removed (grams-per-age-group, RC codes, department order counts) belong to the ORIGINAL
// Generate Menu/Build Menu/Export All Sections export, which this feature reads from but never
// writes back to -- that original file is completely unaffected by any of this.
//
...
// correctly. `dishColumnBySheet` (also from parseWorkbookDishes) is reused as-is to decide the
// deletion boundary -- re-detecting it here would require duplicating the same category/
// dish-column scoring logic, and nothing about column positions has changed between parse time
// and here (this function is the first and only thing to touch them since).
async function restructureAndAppendIngredients(workbook, rowsWithIngredients, dishColumnBySheet = {}) {
  ...
  for (const [sheetName, rows] of bySheet) {
    ...
    if (!dishCol) continue; // parseWorkbookDishes only ever emits rows for a sheet once a dish column was actually found

    // Delete outright, not shift-and-leave-blank -- everything from just past dish name to the
    // sheet's current last column goes. A no-op when dish name already IS the last column (e.g.
    // CEO's real layout has nothing past its second Item column).
    if (sheet.columnCount > dishCol) {
      sheet.spliceColumns(dishCol + 1, sheet.columnCount - dishCol);
    }

    // Ingredients then Allergens are the new rightmost columns -- a plain write, not an
    // insert-and-shift, since the deletion above guarantees nothing occupies them anymore.
    const ingredientsCol = dishCol + 1;
