...
// Used by both the Excel builders below (buildRecipeSheet/buildExtractedRecipeSheet) and the
// preview content-model builders (buildRecipeContentModel/buildExtractedRecipeContentModel) --
// so a numbering/rounding/label decision can never drift between what the real .xlsx export
// shows and what the in-app "preview export" modal shows for the same recipe.
...
function computeProcessTotals(proc) {
  ...
  const wastePct = proc.waste_percent != null ? Math.min(Math.max(proc.waste_percent, 0), 100) : 0;
  const netWeight = Math.round(total * (1 - wastePct / 100) * 100) / 100;
...
async function buildRecipeSheet(workbook, recipe, ingredients, sheetName, options = {}) {
  ...
  if (showTotalQuantity) {
    const total = sumQuantities(ingredients);
  ...
  const prepStepLines = splitLines(prepText);
  ...
  // Photo (square box, A:B -- crop-to-fill photo or empty placeholder) + Presentation/
  // Decoration/Serving (C:D, one line per row like the template). Photo's row-span matches
  // however many rows Presentation actually needs; growBlockToSquare (called once Presentation's
  // own per-line heights are set, below) then grows those rows just enough to make the A:B box
  // square against its fixed column width, without touching row COUNT/content at all.
  ...
  if (recipe.photoBuffer) {
    // Cropped to a square before embedding -- ExcelJS's tl/br anchor always stretches to fill
    // its box, so a genuine "crop to fill, don't distort" requires cropping the pixels first.
    const ext = recipe.photoExt || 'jpeg';
    const croppedBuffer = await squareCropBuffer(recipe.photoBuffer, ext);
...
async function buildExtractedRecipeSheet(workbook, recipe, processes, sheetName, options = {}) {
  const labels = resolveLabels(options);
  const sheet = workbook.addWorksheet(sheetName);
  sheet.columns = [{ width: 29.5 }, { width: 9.2 }, { width: 10 }, { width: 49.7 }];
  sheet.pageSetup = { paperSize: 9, orientation: 'portrait', fitToWidth: 1, fitToHeight: 0, horizontalCentered: true };
...
async function buildExtractedRecipePhotosSheet(workbook, recipe, sheetName, options = {}) {
  const labels = resolveLabels(options);
  const sheet = workbook.addWorksheet(sheetName);
...
async function exportExtractedRecipes(fetchRecipe, recipeIds, savePath, onProgress) {
  ...
  for (const recipeId of recipeIds) {
    ...
    await buildExtractedRecipeSheet(workbook, recipe, processes, sheetName, { labels, targetLanguage });

    if ((recipe.photos || []).length >= 2) {
      const photoSheetName = uniqueSheetName(usedNames, sanitizeSheetName(`${recipe.name} - Photos`));
      await buildExtractedRecipePhotosSheet(workbook, recipe, photoSheetName, { labels, targetLanguage });
    }
  }
...
async function exportRecipes(fetchRecipe, recipeIds, savePath, onProgress) {
  ...
  for (const recipeId of recipeIds) {
    ...
    await buildRecipeSheet(workbook, recipe, ingredients, sheetName, { showTotalQuantity: true, labels, targetLanguage });
  }
  if (onProgress) onProgress('Building Excel file…');
  await workbook.xlsx.writeFile(savePath);
}
...
async function exportScaledRecipe(recipe, ingredients, savePath, options = {}) {
  const workbook = new ExcelJS.Workbook();
  await buildRecipeSheet(workbook, recipe, ingredients, sanitizeSheetName(recipe.name), {
...
async function exportScaledExtractedRecipe(recipe, processes, savePath, options = {}) {
  ...
  const usedNames = new Set();
  const sheetName = uniqueSheetName(usedNames, sanitizeSheetName(recipe.name));
  await buildExtractedRecipeSheet(workbook, recipe, processes, sheetName, options);

  if ((recipe.photos || []).length >= 2) {
    const photoSheetName = uniqueSheetName(usedNames, sanitizeSheetName(`${recipe.name} - Photos`));
    await buildExtractedRecipePhotosSheet(workbook, recipe, photoSheetName, options);
  }
  if (options.onProgress) options.onProgress('Building Excel file…');
  await workbook.xlsx.writeFile(savePath);
}
...
// ============================================================
// In-app export preview ("eye" icon on the Recipe Book/Extractor list rows) -- these two
// return a plain, JSON-safe content model instead of writing to a worksheet, built from the
// exact same shared helpers (splitLines/sumQuantities/computeProcessTotals/resolveLabels) that
// buildRecipeSheet/buildExtractedRecipeSheet themselves call, so a numbering/rounding/label
// decision can never drift between what the real .xlsx shows and what the preview modal shows.
// Deliberately excludes photo bytes (the renderer already has ns.api.getPhoto/getPhotos for
// that) and is never passed a targetLanguage -- the preview always shows the recipe's original
// saved English content, regardless of what's selected in the list screen's export-language
// picker (translation only happens on an actual export).
function buildRecipeContentModel(recipe, ingredients, options = {}) {
  const labels = resolveLabels(options);
  const prepLines = splitLines(recipe.preparation_cooking);
  const presentationLines = splitLines(recipe.presentation_serving);
  ...
  return {
    ...
    header: {
      ...
      wastePercent: recipe.waste_percent != null ? recipe.waste_percent : null,
      netWeight: recipe.yield_notes || '',
    },
    ingredients: (ingredients || []).map(ing => ({
      name: ing.ingredient_name || '', quantity: ing.quantity ?? '', unit: ing.unit || '', method: ing.method || '',
    })),
    totalQuantity: sumQuantities(ingredients),
    preparation: { lines: prepLines, numbered: prepLines.length > 1 },
    presentation: { lines: presentationLines, numbered: presentationLines.length > 1 },
    comment: recipe.comment || '',
    checkedBy: recipe.checked_by || '',
    hasPhoto: !!recipe.photo_path,
  };
}

// Same contract as buildRecipeContentModel above, mirroring buildExtractedRecipeSheet's shape
// instead: one entry per named process (each with its own ingredients/method/totals), and a
// hasSeparatePhotoSheet flag so the preview modal knows whether to also render a "Photos" page
// the way exportExtractedRecipes would (see buildExtractedRecipePhotosSheet) -- 2+ photos.
function buildExtractedRecipeContentModel(recipe, processes, options = {}) {
  const labels = resolveLabels(options);
  ...
  return {
    ...
    processes: (processes || []).map(proc => {
      ...
      return {
        ...
        totalQuantity: total,
        wastePercent: proc.waste_percent != null ? proc.waste_percent : null,
        netWeight,
    ...
    // on the main sheet (buildExtractedRecipeSheet, photos.length <= 1) or above the photo grid
    // on the separate Photos sheet (buildExtractedRecipePhotosSheet, 2+ photos). This flag only
    // tells the caller (the preview renderer) WHERE to place `presentation`, not whether to.
...
module.exports = {
  ...
  DEFAULT_LABELS, buildRecipeContentModel, buildExtractedRecipeContentModel,
};
