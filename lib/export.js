...
const STAFF_ROW_MAP = {
  ...
  // Kept for menus saved before the three fixed beverages replaced it (2026-09-23).
  STAFF_JUICE: { period: 'Lunch', item: 'Juice' },
  STAFF_WATER: { period: 'Lunch', item: 'Water' },
  STAFF_SOFT_DRINK: { period: 'Lunch', item: 'Soft Drink' },
  STAFF_FRESH_JUICE: { period: 'Lunch', item: 'Fresh Juice' },
...
// ============================================================
// School sections (Daycare / KG-LP / MS-UP) -- Category/RC/Item Name
// plus ONE plain Quantity column (e.g. "150g", "05 option") -- headcount/pax
// columns removed entirely, per the chef's own request (see conversation
// notes: the old layout had a unit-labeled quantity cell PLUS a blank
// B=/L=/P= headcount cell per age group; only the quantity survives).
//
// A single value, not one per age group, because portion size is no longer
// stored per age group at all: getPortion() (see fetchGeneratedMenuExportData
// in main.js) resolves to category_portion_defaults, which is keyed by
// (category, SECTION) -- every age group within a section shares the same
// section_id, so every age group is architecturally guaranteed to resolve
// to the identical value for a given item. (An earlier version of this sheet
// looped over every age group and joined "<name>: <qty>" with " / ", which
// just re-printed that one identical value once per age group -- e.g. MS-UP's
// 4 age groups produced "MS - Girls: 05 option / MS - Boys: 05 option / ...",
// unreadable and pointless since all 4 segments always matched.)
// ============================================================
...
function buildSchoolSheet(workbook, exportData, sheetName) {
  ...
  // Columns: 1=Meal Period, 2=Category, 3=RC, 4=Item Name, 5=Quantity (one plain value, same
  // across every age group -- see the section comment above). Header text is literally
  // "Quantity" -- lib/menuIngredients.js's Menu Ingredients Generator detects this exact text
  // as its School-layout marker (scanRowMarkers), so renaming it again means updating that too.
  sheet.columns = [
    { width: 16 }, { width: 20 }, { width: 14 }, { width: 60 }, { width: 34 },
  ];
  const totalCols = 5;
  ...
  for (const day of days) {
    ...
    const headerRow = sheet.addRow([
      formatDateDDMMYYYY(day.menu_date), day.day_of_week.toUpperCase(), 'RC', '', 'Quantity',
    ]);
    headerRow.height = 26;
    ...
    items.forEach((it, idx) => {
      // Static fallback (used as-is for a daily-repeating item, and as the initial value for
      // every other item until the formula block below overwrites it). Every age group in
      // `ageGroups` belongs to this same section, and getPortion() is keyed by (category,
      // section) only, so any one of them gives the value every other one would too --
      // ageGroups[0] is picked arbitrarily, not "the first that happens to have data".
      const portion = ageGroups.length > 0 ? getPortion(it.item_id, ageGroups[0].id) : null;
      const quantityText = portion ? `${portion.quantity}${portion.unit}` : '';
      const rowValues = ['', it.category_name, it.rc_code || 'NEW', it.name, quantityText];
      ...
      if (it.is_daily_repeating !== 1) {
        ...
        // One plain lookup, not one per age group -- the lookup TABLE (buildListsSheetFromData)
        // still has one quantity column per age group, but every one of them holds the same
        // value (see the section-level getPortion comment above), so reading just the first
        // (colIndex 3) is enough; no need to concatenate the rest.
      ...
      // so bolding on category alone put a thick line after nearly every row (that's what
      // NO_CATEGORY_LINES/DAYCARE was papering over: one item per category there, so EVERY row
      // was a category boundary). Meal periods group several categories together, so this reads
      // as one clean separator between periods instead, with items/categories inside a period
      // sharing only the plain thin border every cell already gets.
...
// ============================================================
// Staff (buffet order form) -- Meal Period | Item Type | RC | Item | Weight/Unit -- the
// per-department order-count columns (Admin/LP/KG/UPB/M&SB/M&SG/UPG) are removed entirely, per
// the chef's own request; Weight/Unit already was the Quantity column, unchanged.
// ============================================================
function buildStaffSheet(workbook, exportData, sheetName) {
  ...
  const sheet = workbook.addWorksheet(sheetName.slice(0, 31));
  // Columns: 1=Meal Period, 2=Item Type, 3=RC, 4=Item, 5=Weight/Unit
  sheet.columns = [
  ...
  for (const day of days) {
    ...
    const headerRow = sheet.addRow([
      day.day_of_week.toUpperCase(), '', 'RC', formatDateDDMMYYYY(day.menu_date), 'Weight/Unit',
    ]);
    ...
    items.forEach((it, idx) => {
      ...
      const row = sheet.addRow([map.period, itemLabel, it.rc_code || 'NEW', it.name, weight]);
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        cell.border = allBorders;
        cell.alignment = centerMiddle;
