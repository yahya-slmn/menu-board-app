...
async function buildRecipeSheet(workbook, recipe, processes, sheetName, options = {}) {
  ...
  for (const proc of processes) {
    ...
    if (procIngredients.length > 0) {
      ...
      const procTotalRow = sheet.addRow([labels.totalQuantity, `${procTotal} G`]);
      ...
      const procNetRow = sheet.addRow([labels.netWeight, `${procNetWeight} G`]);
  ...
  const totalRow = sheet.addRow([labels.totalQuantity, `${sumQuantities(allIngredients)} G`]);
