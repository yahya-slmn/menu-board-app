...
// Detects RTL content directly (Hebrew, Arabic, and the scripts that share its Unicode block --
// Urdu/Farsi/Pashto/Kurdish all use Arabic script -- plus Syriac/Thaana/N'Ko) rather than
// remembering which language the chef picked at extraction time: the recipe's own saved text is
// the source of truth, so this stays correct even if she edits a field into a different script
// later, and needs no per-language lookup table. Excel/ExcelJS has no equivalent to a browser's
// dir="auto", so buildExtractedRecipeSheet/buildExtractedRecipePhotosSheet call this once per
// recipe and flip specific data cells' alignment to 'right' when it's true -- deliberately not a
// full mirrored (rightToLeft) worksheet layout, which would upend a lot of layout built assuming
// LTR flow for comparatively little benefit here.
const RTL_CHAR_RE = /[\u0590-\u05FF\u0600-\u06FF\u0700-\u074F\u0750-\u077F\u0780-\u07BF\u07C0-\u07FF\u08A0-\u08FF\uFB1D-\uFDFF\uFE70-\uFEFF]/;
function isRtlRecipe(recipe) {
  return RTL_CHAR_RE.test(recipe.name || '');
}
...
function buildRecipeSheet(workbook, recipe, ingredients, sheetName, options = {}) {
  ...
  const wasteRow = sheet.addRow([null, null, null, `Waste: ${recipe.waste_percent != null ? recipe.waste_percent + '%' : ''}`]);
  ...
  const r5 = sheet.addRow([`Country \\ Origin: ${recipe.country_origin || ''}`, null, null, `Net Weight: ${recipe.yield_notes || ''}`]);
  ...
  if (prepStepLines.length > 1) {
    prepStepLines.forEach((line, idx) => {
      ...
      const stepRow = sheet.addRow([`${idx + 1}. ${line}`]);
      sheet.mergeCells(stepRow.number, 1, stepRow.number, 4);
      sheet.getCell(`A${stepRow.number}`).font = { size: 13, name: RECIPE_FONT };
      sheet.getCell(`A${stepRow.number}`).alignment = { horizontal: 'left', vertical: 'top', wrapText: true };
    ...
    const prepRowHeight = Math.max(3, Math.ceil(prepText.length / 90));
    const prepValueRow = sheet.addRow([prepText]);
    sheet.mergeCells(prepValueRow.number, 1, prepValueRow.number, 4);
    prepValueRow.height = Math.max(20, prepRowHeight * 15);
  ...
  for (let i = 0; i < presentationRowCount; i++) {
    ...
    sheet.getCell(rowNum, 3).value = presentationLines[i] ? (numberPresentationLines ? `${i + 1}. ${presentationLines[i]}` : presentationLines[i]) : '';
...
function addOptionalFieldRow(sheet, label, value, align) {
  ...
  const row = sheet.addRow([`${label}: ${trimmed}`]);
  sheet.mergeCells(row.number, 1, row.number, 4);
  sheet.getCell(row.number, 1).font = { size: 13, name: RECIPE_FONT };
  sheet.getCell(row.number, 1).alignment = { horizontal: align || 'left' };
  return row;
}

function buildExtractedRecipeSheet(workbook, recipe, processes, sheetName) {
  const sheet = workbook.addWorksheet(sheetName);
  sheet.columns = [{ width: 29.5 }, { width: 9.2 }, { width: 10 }, { width: 49.7 }];
  sheet.pageSetup = { paperSize: 9, orientation: 'portrait', fitToWidth: 1, fitToHeight: 0, horizontalCentered: true };
  sheet.views = [{ showGridLines: false }];

  // Cell-level only (see isRtlRecipe) -- the sheet's own column order/template labels ("RECIPE
  // FOR:", "Total Quantity", "Method:", etc., all fixed English) stay exactly where they are;
  // only cells holding this recipe's own extracted text right-align when it reads RTL.
  const isRtl = isRtlRecipe(recipe);
  const dataAlign = isRtl ? 'right' : 'left';

  // ---- Header: recipe name (always) + EX code (always) + one row per optional field.
  const nameRow = sheet.addRow([null, 'RECIPE FOR:', null, recipe.name]);
  ...
  const codeRow = sheet.addRow([`EX Code: ${recipe.code}`]);
  ...
  addOptionalFieldRow(sheet, 'Quantity Produced', recipe.quantity_produced, dataAlign);
  addOptionalFieldRow(sheet, 'Prepared By', recipe.prepared_by, dataAlign);
  addOptionalFieldRow(sheet, 'Category', recipe.category, dataAlign);
  addOptionalFieldRow(sheet, 'Country \\ Origin', recipe.country_origin, dataAlign);
  addOptionalFieldRow(sheet, 'Date', formatDateDDMMYYYY(recipe.date_created), dataAlign);
  ...
  const yieldRow = sheet.addRow([`Net Weight: ${recipe.yield_notes || ''}`]);
  ...
  for (const proc of processes) {
    const headingRow = sheet.addRow([proc.name || '']);
    ...
    sheet.getCell(`A${headingRow.number}`).alignment = { horizontal: dataAlign };
    ...
    if (procIngredients.length > 0) {
      ...
      const header = sheet.addRow(['INGREDIENTS', 'Quantity', 'Unit', 'Note']);
      header.height = 20;
      header.eachCell({ includeEmpty: true }, cell => {
      ...
      procIngredients.forEach(ing => {
        const row = sheet.addRow([ing.display_name || ing.ingredient_name, ing.quantity ?? '', ing.unit || '', ing.method || '']);
      ...
      if (proc.quantity_produced) {
        const procQtyRow = sheet.addRow(['Quantity Produced', proc.quantity_produced]);
      ...
      const procTotalRow = sheet.addRow(['Total Quantity', procTotal]);
      ...
      if (proc.waste_percent != null) {
        const procWasteRow = sheet.addRow(['Waste', `${proc.waste_percent}%`]);
      ...
      const procNetRow = sheet.addRow(['Net Weight', procNetWeight]);
    ...
    if (methodLines.length > 0) {
      const methodLabelRow = sheet.addRow(['Method:']);
      ...
      if (methodLines.length > 1) {
        ...
        valueRow.height = Math.max(20, Math.ceil(methodText.length / 90) * 15);
  ...
  const totalRow = sheet.addRow(['Total Quantity', Math.round(rawTotal * 100) / 100]);
  totalRow.getCell(1).font = { bold: true, size: 13, name: RECIPE_FONT };
  totalRow.getCell(2).font = { bold: true, size: 13, name: RECIPE_FONT };
  totalRow.getCell(2).alignment = { horizontal: 'center' };
  ...
  if (photos.length <= 1) {
    ...
    if (hasPresentation) {
      ...
      presentationLines.forEach((line, i) => {
        const rowNum = blockStartRow + 1 + i;
        sheet.mergeCells(rowNum, 3, rowNum, 4);
        sheet.getCell(rowNum, 3).value = numberLines ? `${i + 1}. ${line}` : line;
        sheet.getCell(rowNum, 3).font = { size: 13, name: RECIPE_FONT };
        sheet.getCell(rowNum, 3).alignment = { horizontal: dataAlign };
      });
    }
  ...
  const commentRow = addOptionalFieldRow(sheet, 'Comment', recipe.comment, dataAlign);
  if (commentRow) frameRecipeRegion(sheet, commentRow.number, 1, commentRow.number, 4, recipeMediumBorder, false);
  const checkedByRow = addOptionalFieldRow(sheet, 'Checked By', recipe.checked_by, dataAlign);
...
function buildExtractedRecipePhotosSheet(workbook, recipe, sheetName) {
  const sheet = workbook.addWorksheet(sheetName);
  ...
  // Same cell-level-only approach as buildExtractedRecipeSheet -- see isRtlRecipe.
  const dataAlign = isRtlRecipe(recipe) ? 'right' : 'left';
  ...
  sheet.getCell(`B${nameRow.number}`).font = { bold: true, underline: true, size: 16, name: RECIPE_FONT };
  sheet.getCell(`B${nameRow.number}`).alignment = { horizontal: 'center', vertical: 'middle' };
  sheet.getCell(`D${nameRow.number}`).value = 'Photos & Presentation';
  ...
  if (presentationLines.length > 0) {
    const labelRow = sheet.addRow(['Presentation, Decoration & Serving Instructions:']);
    ...
    presentationLines.forEach((line, i) => {
      ...
      sheet.getCell(rowNum, 1).value = numberLines ? `${i + 1}. ${line}` : line;
...
// fetchRecipe(recipeId) -> Promise<{ recipe, processes }> -- mirrors exportRecipes' contract
// below but with `processes` (each carrying its own `ingredients`) instead of a flat
// `ingredients` array, since extracted_recipe_processes has no Recipe Book equivalent. `recipe`
// also carries `photos` (an array of { buffer, ext }, in gallery order) -- 0 or 1 stays on the
// main sheet as before; 2+ gets its own additional sheet (see buildExtractedRecipePhotosSheet).
async function exportExtractedRecipes(fetchRecipe, recipeIds, savePath) {
  const workbook = new ExcelJS.Workbook();
  const usedNames = new Set();
  for (const recipeId of recipeIds) {
    const { recipe, processes } = await fetchRecipe(recipeId);
    const sheetName = uniqueSheetName(usedNames, sanitizeSheetName(recipe.name));
    buildExtractedRecipeSheet(workbook, recipe, processes, sheetName);

    if ((recipe.photos || []).length >= 2) {
      const photoSheetName = uniqueSheetName(usedNames, sanitizeSheetName(`${recipe.name} - Photos`));
      buildExtractedRecipePhotosSheet(workbook, recipe, photoSheetName);
    }
  }
  await workbook.xlsx.writeFile(savePath);
}
...
async function exportScaledExtractedRecipe(recipe, processes, savePath) {
  const workbook = new ExcelJS.Workbook();
  const usedNames = new Set();
  const sheetName = uniqueSheetName(usedNames, sanitizeSheetName(recipe.name));
  buildExtractedRecipeSheet(workbook, recipe, processes, sheetName);
