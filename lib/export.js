...
// Decoration/Serving block are all omitted entirely when blank.
// ============================================================
...
function addOptionalFieldRow(sheet, label, value) {
  ...
  sheet.getCell(row.number, 1).alignment = { horizontal: 'left' };
  return row;
}
...
function buildExtractedRecipeSheet(workbook, recipe, processes, sheetName) {
  ...
  addOptionalFieldRow(sheet, 'Quantity Produced', recipe.quantity_produced);
  addOptionalFieldRow(sheet, 'Prepared By', recipe.prepared_by);
  addOptionalFieldRow(sheet, 'Category', recipe.category);
  addOptionalFieldRow(sheet, 'Country \\ Origin', recipe.country_origin);
  addOptionalFieldRow(sheet, 'Date', formatDateDDMMYYYY(recipe.date_created));
  addOptionalFieldRow(sheet, 'Waste', recipe.waste_percent != null ? `${recipe.waste_percent}%` : '');

  // Net Weight/Yield is always shown -- computed live from ingredients + waste, so it's
  // effectively never truly blank the way the fields above can be.
  ...
  for (const proc of processes) {
    ...
    if (methodLines.length > 0) {
      ...
      if (methodLines.length > 1) {
        ...
        sheet.getCell(`A${valueRow.number}`).alignment = { horizontal: 'left', vertical: 'top', wrapText: true };
      }
      sectionLastRow = sheet.rowCount;
    }
  ...
  // ---- Total Quantity: aggregate across every process's ingredients combined, same
  // convention as buildRecipeSheet's (and matches the form's live Yield calc, which sums
  // across every process too -- see updateYieldCalculation in renderer.js).
  ...
  const commentRow = addOptionalFieldRow(sheet, 'Comment', recipe.comment);
  if (commentRow) frameRecipeRegion(sheet, commentRow.number, 1, commentRow.number, 4, recipeMediumBorder, false);
  const checkedByRow = addOptionalFieldRow(sheet, 'Checked By', recipe.checked_by);
...
// `ingredients` array, since extracted_recipe_processes has no Recipe Book equivalent.
...
module.exports = {
  ...
  exportExtractedRecipes, sanitizeSheetName, SECTION_DISPLAY_NAMES,
};
