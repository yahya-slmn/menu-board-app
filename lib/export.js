...
function buildSchoolSheet(workbook, exportData, sheetName) {
  ...
  sheet.columns = [{ width: 16 }, { width: 20 }, { width: 60 }];
  const totalCols = 3;

  for (const day of days) {
    const items = day.items.slice().sort((a, b) => (a.period_order - b.period_order) || (a.cat_order - b.cat_order));

    const headerRow = sheet.addRow([formatDateDDMMYYYY(day.menu_date), day.day_of_week.toUpperCase(), '']);
    headerRow.height = 26;
...
function buildStaffSheet(workbook, exportData, sheetName) {
  ...
  for (const day of days) {
    ...
    const headerRow = sheet.addRow([day.day_of_week.toUpperCase(), '', formatDateDDMMYYYY(day.menu_date)]);
...
// ============================================================
// Columns: 1=Meal Period, 2=Item Type, 3=Item(Steffen), 4=Item(Khodary) -- no RC columns since
// 2026-09-24. The person names in the header row are how lib/menuIngredients.js recognises CEO.
...
function buildCeoSheet(workbook, exportData, sheetName) {
  ...
  const sheet = workbook.addWorksheet(sheetName.slice(0, 31));
  sheet.columns = [{ width: 14 }, { width: 16 }, { width: 42 }, { width: 42 }];
  const totalCols = 4;
  ...
  for (const day of days) {
    ...
    const headerRow = sheet.addRow([
      day.day_of_week.toUpperCase(), formatDateDDMMYYYY(day.menu_date), CEO_PERSONS[0], CEO_PERSONS[1],
    ]);
    headerRow.height = 22;
    ...
    items.forEach((it, idx) => {
      ...
      // CEO_BREAD_EXCLUDED_PERSON never gets Bread -- his side of that row stays blank.
      const row = sheet.addRow([map.period, map.item, isBreadRow ? '' : it.name, it.name]);
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        cell.border = allBorders;
        cell.alignment = centerMiddle;
        cell.font = { name: 'Calibri', size: 12 };
      ...
      if (it.is_daily_repeating !== 1) {
        const list = definedNameFor('CEO', it.category_code);
        row.getCell(4).dataValidation = { ...VALIDATION_STYLE, formulae: [list] };
        if (!isBreadRow) row.getCell(3).dataValidation = { ...VALIDATION_STYLE, formulae: [list] };
      }
...
function buildCeoTemplateSheet(workbook, listsData, sectionCode, sheetName, days) {
  ...
  for (const day of days) {
    const headerRow = sheet.addRow([day.weekday.toUpperCase(), formatDateDDMMYYYY(day.date), CEO_PERSONS[0], CEO_PERSONS[1]]);
    headerRow.height = 22;
    headerRow.eachCell(cell => {
      fillCell(cell, GREEN);
      cell.font = { name: 'Calibri', size: 12, bold: true, color: { argb: 'FFFFFFFF' } };
    ...
    specs.forEach((spec, idx) => {
      ...
      const row = sheet.addRow([map.period, map.item, isBreadRow ? '' : name, name]);
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        cell.border = allBorders;
        cell.alignment = centerMiddle;
        cell.font = { name: 'Calibri', size: 12 };
      ...
      if (spec.definedName) {
        ...
        if (!isBreadRow) row.getCell(3).dataValidation = { ...VALIDATION_STYLE, formulae: [spec.definedName] };
      }
