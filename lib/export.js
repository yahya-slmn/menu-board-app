...
// plus ONE combined Quantity column across every age group (e.g.
// "KG: 150g / LP: 200g") -- headcount/pax columns removed entirely,
// per the chef's own request (see conversation notes: the old layout
// had a unit-labeled quantity cell PLUS a blank B=/L=/P= headcount
// cell per age group; only the quantity survives, merged into one
// cell rather than kept as separate per-age-group columns).
// ============================================================
...
function buildSchoolSheet(workbook, exportData, sheetName) {
  ...
  // Columns: 1=Meal Period, 2=Category, 3=RC, 4=Item Name, 5=Quantity (combined across every
  // age group). Header text is literally "Quantity" -- lib/menuIngredients.js's Menu Ingredients
  // Generator detects this exact text as its School-layout marker (scanRowMarkers), so renaming
  // it again means updating that too.
  sheet.columns = [
  ...
  for (const day of days) {
    ...
    items.forEach((it, idx) => {
      ...
      // every other item until the formula block below overwrites it) -- always includes every
      // age group's own label, even when a portion is missing for one of them (an empty
      // "LP: " segment reads oddly but honestly, same as the old per-cell version leaving that
      // one cell blank; not worth a conditional-join formula's added complexity for this edge case).
      const quantityText = ageGroups.map(ag => {
        const portion = getPortion(it.item_id, ag.id);
        return `${ag.name}: ${portion ? `${portion.quantity}${portion.unit}` : ''}`;
      }).join(' / ');
      ...
      if (it.is_daily_repeating !== 1) {
        ...
        // One combined formula, not one cell per age group -- string-concatenates each age
        // group's own "<name>: "&IFERROR(INDEX(...),"") lookup (same lookupFormula/colIndex
        // convention every other formula in this file already uses -- the lookup TABLE's own
        // column structure is untouched by any of this, see lookupFormula's own comment) with
        // " / " between them, so the cell auto-updates for every age group at once when she
        // picks a different item from the dropdown, exactly like the old per-cell formulas did
        // individually.
        if (ageGroups.length > 0) {
          const parts = ageGroups.map((ag, agIdx) => `"${ag.name}: "&${lookupFormula(nameCellAddr, spec, 3 + agIdx)}`);
          row.getCell(5).value = { formula: parts.join('&" / "&') };
        }
      }
