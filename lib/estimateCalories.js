...
// of { name, category, protein, ingredients? } -- `ingredients` is only present when main.js
// found a real matching recipe for that dish (see findRecipeIngredientsForDishName), so the model
// can ground its estimate in an actual composition instead of guessing from the name alone. This
// module has no idea which menu_items row any given position corresponds to; building that array
// and mapping the returned estimates back onto specific rows is main.js's job (see the
// estimate-missing-calories handler there), same split translateTexts/extractRecipeFromFile
// already establish for their own Edge Function calls.
