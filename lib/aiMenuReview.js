...
//   KG-LP), and Staff's shared Main (KG-LP + Daycare Lunch Main) and Breakfast (the school AM
//   Snacks). Changing the SOURCE pick updates every copy (propagationTargets), the same way the
//   engine built them.
...
const { eligibleItemsSupabase } = require('./generator');
...
const DISH_COLUMNS = 'id, run_id, name, category_code, section_codes, protein_code, sauce_type, carb_type, dish_concept, am_snack_style, description, key_ingredients, safety_scan, dup_match_item_id, dup_match_type, resolution, edited, resolved_item_id';
...
const ATTR_LABEL = { protein_code: 'protein type', sauce_type: 'sauce style', carb_type: 'starch type', dish_concept: 'dish type', am_snack_style: 'AM Snack style' };
...
async function listRuns() {
  const { data, error } = await supabase.from('ai_menu_runs')
    .select('id, label, created_by, start_date, end_date, num_weekdays, status, warnings, created_at, approved_at, approved_by')
...
function propagationTargets(pick, picks) {
  ...
  if (hasValue && pick.category_code === 'LUNCH_MAIN' && ['KG_LP', 'DAYCARE'].includes(pick.section_code)) {
...
async function listReplacementOptions({ runId, pickId }) {
  ...
  // KG-LP's coupled Lunch Main / Starch is served to MS-UP too: offer only items both can have.
  if (pick.section_code === 'KG_LP' && ['LUNCH_MAIN', 'LUNCH_STARCH'].includes(pick.category_code)) {
    const partner = new Set((await eligibleItemsSupabase(getSectionByCode('MS_UP').id, category.id)).map((i) => i.id));
...
async function createChefDish(runId, categoryCode, sections, fields, origin) {
  ...
  const safety = checkDishSafety({ ...typed, section_codes: sections });
  if (!safety.ok || error) {
...
async function replacePick({ runId, pickId, replacement }) {
  ...
  if (replacement.newDish) {
    const created = await createChefDish(runId, pick.category_code, sections, replacement.newDish, replacement.origin || 'chef');
...
async function updateDish({ runId, dishId, fields }) {
  ...
  const safety = checkDishSafety({ ...typed, section_codes: current.section_codes });
...
async function suggestForPick({ runId, pickId }) {
  ...
  const request = buildDishRequest({ ...pool, sections }, [{ count: 3, ...(Object.keys(fixed).length ? { fixed } : {}) }], proteinCodes,
  ...
  for (const raw of raws) {
    ...
    if (error) continue;
    const safety = checkDishSafety({ ...dish, section_codes: sections });
  ...
  return { candidates, rejected, keptAttributes: fixed };
}
