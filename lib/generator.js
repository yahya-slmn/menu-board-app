...
const { labelOf, mixOrder } = require('./createdByMix');
...
const SECTION_SLOTS = {
  ...
  // CEO v2 (2026-09-25): Breakfast = Main Dish, Juice, Yogurt; Lunch = Main Dish, Salad, Juice, Snack,
  // Bread. CEO_RAW_VEG retired forward-only (its dishes stay, never picked; old menus keep the row).
  // CEO_FRUITS IS the Snack category (renamed, code kept). The export's "Protein" row is only a second
  // label on the Lunch Main Dish's own cell (lib/export.js), never a pick.
  CEO: [
    ['CEO_BREAKFAST_MAIN', 1, {}], ['CEO_BREAKFAST_JUICE', 1, {}], ['CEO_YOGURT', 1, {}],
    ['CEO_LUNCH_MAIN', 1, {}], ['CEO_SALAD', 1, {}], ['CEO_LUNCH_JUICE', 1, {}],
    ['CEO_FRUITS', 1, {}], ['CEO_BREAD', 1, {}],
  ],
};
...
// How many of a Staff slot's daily picks are forced-in shares (above / below), i.e. not Staff's own
// choice -- the Created By mix pool check counts only the choices (Main 7 - 5 = 2, Breakfast 6 - 2 = 4).
const STAFF_FORCED_PER_DAY = { STAFF_MAIN: 5, STAFF_BREAKFAST: 2 };
...
async function sectionItemPoolSupabase(sectionId) {
  ...
  const { data: items, error: itemsErr } = await supabase
    .from('menu_items')
    // created_by_label: read by the Menu Planner's optional Created By mix (createdByMix below).
    .select('id, name, rc_code, protein_type_id, is_daily_repeating, sauce_type, carb_type, dish_concept, am_snack_style, category_id, is_active, created_by_label')
    .eq('is_active', 1)
    .in('id', itemIds);
...
// The Menu Planner's Created By mix pool check, before generating: per section + category, the dishes
// each Created By value has that the engine could choose from (the section's pool -- or, for a shared
// category, the dishes both sections have -- minus chicken / beef snacks) and how many choices the run
// makes there (Staff's forced shares excluded). fixedDaily: the slot is a daily dish, the mix can't
// act. follows: in an All Sections run this section copies its partner's shared picks.
async function createdByMixPoolCheck({ sectionCodes, numWeekdays, allSections }) {
  const ORDER = ['DAYCARE', 'KG_LP', 'MS_UP', 'STAFF', 'CEO'];
  const pools = new Map();
  const poolOf = async (code) => {
    if (!pools.has(code)) pools.set(code, await sectionItemPoolSupabase(getSectionByCode(code).id));
    return pools.get(code);
  };
  const rows = [];
  const labels = new Set();
  for (const sectionCode of sectionCodes) {
    const pool = await poolOf(sectionCode);
    const partners = couplingPartners(sectionCode);
    for (const [categoryCode, count, options] of SECTION_SLOTS[sectionCode]) {
      const category = getCategoryByCode(categoryCode);
      const row = { section: sectionCode, category: categoryCode, categoryName: category?.name || categoryCode, picks: 0, byLabel: {} };
      let catPool = pool.filter(it => it.category_id === category?.id
        && !snackLunchOnlyHit(categoryCode, getProteinById(it.protein_type_id)?.code, [['name', it.name]]));
      if (options.fixedDaily || catPool.some(it => it.is_daily_repeating === 1)) { rows.push({ ...row, fixedDaily: true }); continue; }
      const partner = partners[categoryCode];
      if (partner && allSections && ORDER.indexOf(partner) < ORDER.indexOf(sectionCode)) { rows.push({ ...row, follows: partner }); continue; }
      if (partner) {
        const partnerIds = new Set((await poolOf(partner)).map(it => it.id));
        catPool = catPool.filter(it => partnerIds.has(it.id));
        row.sharedWith = partner;
      }
      row.picks = (count - (sectionCode === 'STAFF' ? STAFF_FORCED_PER_DAY[categoryCode] || 0 : 0)) * numWeekdays;
      for (const it of catPool) {
        const l = labelOf(it);
        row.byLabel[l] = (row.byLabel[l] || 0) + 1;
        if (l) labels.add(l);
      }
      rows.push(row);
    }
  }
  return { rows, labels: [...labels].sort((a, b) => a.localeCompare(b)) };
}
...
class MenuGenerator {
  ...
  // - createdByMix: { [created_by_label]: percent } summing to 100 ('' = not set) -- the Menu Planner's
  //   optional Created By mix, passed ONLY by Generate Menu / Export All Sections (never Build Menu's
  //   Auto-Fill or the AI Menu Generator). A best-effort ordering bias among dishes every rule already
  //   allows; null (the default) leaves every pick exactly as without it.
  constructor({ draftPools = null, partnerPicks = null, dayTheme = null, createdByMix = null } = {}) {
    this.warnings = [];
    this._draftPools = draftPools;
    this._partnerPicks = partnerPicks;
    this._dayTheme = dayTheme;
    this._createdByMix = createdByMix;
    // Created By mix bookkeeping, per `${sectionCode}|${categoryCode}` (only kept while a mix is set):
    // picks chosen so far by Created By value, and every dish the category offered, by value (report).
    this._mixTally = new Map();
    this._mixPool = new Map();
  }

  // What the Created By mix did this run: [{ section, category, picks, byLabel, pool }] -- see
  // lib/createdByMix.js summarizeMixReport. Empty when no mix was set.
  createdByMixReport() {
    return [...this._mixTally.entries()].map(([key, tally]) => {
      const [section, category] = key.split('|');
      return { section, category, picks: [...tally.values()].reduce((a, b) => a + b, 0),
        byLabel: Object.fromEntries(tally), pool: Object.fromEntries([...(this._mixPool.get(key) || [])].map(([l, ids]) => [l, ids.size])) };
    });
  }
  ...
  // mixKey (`${sectionCode}|${categoryCode}`, from _pickItems): with a Created By mix set, the fresh
  // candidates (not served in NO_REPEAT_DAYS) are reordered toward the mix (lib/createdByMix.js
  // mixOrder); the rest keep this order after them. Without a mix or a key the order is unchanged.
  _scoreAndSort(sectionId, candidates, onDate, mixKey = null) {
    ...
    if (!this._createdByMix || !mixKey) return scored;
    return mixOrder(scored, this._createdByMix, this._mixTally.get(mixKey) || new Map(), NO_REPEAT_DAYS);
  }
  ...
  async _pickItems(sectionId, categoryId, onDate, count, options, excludeIds, sectionCode, categoryCode, overridePool = null, forceInclude = [], avoidProteinIds = null) {
    ...
    const style = this._styleMixTracker(options.styleMix, sectionCode, onDate, categoryCode);
    const mixKey = this._createdByMix ? `${sectionCode}|${categoryCode}` : null;
    if (mixKey) {
      // Every dish this category offered during the run (a snack's pool is one style a day, so the
      // first day's pool alone would undercount), by Created By value.
      if (!this._mixPool.has(mixKey)) this._mixPool.set(mixKey, new Map());
      const byLabel = this._mixPool.get(mixKey);
      for (const c of pool) {
        if (c.is_daily_repeating === 1) continue;
        if (!byLabel.has(labelOf(c))) byLabel.set(labelOf(c), new Set());
        byLabel.get(labelOf(c)).add(c.id);
      }
    }
    ...
    if (composition && composition.length) {
      for (const rule of composition) {
        ...
        const scoredPool = this._scoreAndSort(sectionId, pool, onDate, mixKey);
        let added = 0;
    ...
    const scored = this._scoreAndSort(sectionId, remaining, onDate, mixKey);
    ...
    if (mixKey) {
      // Only real choices count toward the mix: not the dishes forced in from another section.
      const forced = new Set(forceInclude.map(c => c.id));
      const tally = this._mixTally.get(mixKey) || new Map();
      for (const c of chosen) if (!forced.has(c.id)) tally.set(labelOf(c), (tally.get(labelOf(c)) || 0) + 1);
      this._mixTally.set(mixKey, tally);
    }
    return { ids: chosen.map(c => c.id), names: chosen.map(c => c.name), items: chosen };
  }
...
module.exports = { MenuGenerator, SECTION_SLOTS, STAFF_FORCED_PER_DAY, createdByMixPoolCheck, AM_SNACK_STYLE_BY_PATTERN, SNACK_STYLE_BY_PATTERN, snackStyleFor, SECTION_COUPLINGS, couplingPartners, eligibleItemsSupabase, sectionItemPoolSupabase, schoolDaysFrom, schoolDayCountBetween };
