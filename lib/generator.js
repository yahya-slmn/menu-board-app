...
class MenuGenerator {
  ...
  async persistMenu(sectionCode, label, startDate, resultDays, batchId = null) {
    const sectionId = this._sectionId(sectionCode);
    const slots = SECTION_SLOTS[sectionCode];
  ...
  async generate(sectionCode, label, startDate, numWeekdays, batchId = null) {
    const { resultDays } = await this.computeMenu(sectionCode, startDate, numWeekdays);
    const menuId = await this.persistMenu(sectionCode, label, startDate, resultDays, batchId);
    return { menuId, resultDays };
  }
}

module.exports = { MenuGenerator, SECTION_SLOTS, eligibleItemsSupabase, sectionItemPoolSupabase, schoolDaysFrom };
