...
const SECTION_SLOTS = {
  ...
  STAFF: [
    // 6 breakfast dishes (incl. 1 meat-free -- vegan or vegetarian, varied concepts), THEN the juice pick
    ['STAFF_BREAKFAST', 6, {
    ...
    // category's daily-repeating item, never a rotating pick (fixedDaily).
    ['STAFF_FRUIT_BASKET', 1, {}],
    ['STAFF_WATER', 1, { fixedDaily: true }], ['STAFF_SOFT_DRINK', 1, { fixedDaily: true }], ['STAFF_FRESH_JUICE', 1, { fixedDaily: true }],
    // lunch box: 1 meat protein + 1 meat-free (vegan or vegetarian), then 1 salad from its own pool
    ['STAFF_LUNCHBOX', 2, {
      composition: [
...
class MenuGenerator {
  ...
  async _pickThemed(sectionId, onDate, count, options, excludeIds, sectionCode, categoryCode, themedPool, regularPool, cuisine, forceInclude = [], avoidProteinIds = null) {
    ...
    const tiers = [
      ...
      { pool: themedPool, relax: true, regular: false, sameProtein: true },
      { pool: regularPool, relax: true, regular: true, sameProtein: true },
    ];
    const ok = (c, t) => {
  ...
  _draftPool(categoryCode) {
    return (this._draftPools && this._draftPools[categoryCode]) || null;
  }
  ...
  async computeMenu(sectionCode, startDate, numWeekdays) {
    ...
    if (partnerCode) {
      ...
      for (const catCode of COUPLED_LUNCH_CATEGORIES) {
        // A draft pool for a coupled category is built for both sections at once, so it IS the
        // shared pool -- no intersection needed.
        const draftPool = this._draftPool(catCode);
        if (draftPool) { coupledPool[catCode] = draftPool; continue; }
    ...
    for (const { date, weekday } of schoolDaysFrom(startDate, numWeekdays)) {
      ...
      for (const [catCode, count, options] of slots) {
        ...
        if (AM_SNACK_ROTATION_SECTIONS.includes(sectionCode) && catCode === AM_SNACK_ROTATION_CATEGORY) {
          ...
          if (themeFallback) {
            // National Day: the themed dishes of today's style first, then regular ones of today's
            // style (the style rotation outranks the theme, like every other rule).
            overridePool = stylePool;
            const regularStyle = themeFallback.filter(it => it.am_snack_style === requiredStyle);
            themeFallback = regularStyle.length ? regularStyle : themeFallback;
          } else if (stylePool.length === 0) {
        ...
        if (isCoupled && partnerPicksForDay && partnerPicksForDay.length) {
          ...
          try {
            const split = this._draftPool(catCode) ? this._themeSplit(coupledPool[catCode], date) : { pool: coupledPool[catCode], fallback: null };
            if (split.fallback) {
...
module.exports = { MenuGenerator, SECTION_SLOTS, AM_SNACK_STYLE_BY_PATTERN, eligibleItemsSupabase, sectionItemPoolSupabase, schoolDaysFrom, schoolDayCountBetween };
