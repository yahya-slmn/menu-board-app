// ============================================================
// Misk school-wide policy: no nuts, sesame, or their derived ingredients in ANY suggested
// ingredient list or generated recipe. This is the post-processing safety net behind the
// prompt-level instruction in supabase/functions/suggest-dish-ingredients/index.ts and
// generate-dish-recipes/index.ts -- an LLM following instructions well most of the time still
// isn't a hard guarantee, so every suggestion/recipe gets scanned here regardless of how the
// prompt was worded, and anything matching is stripped before it ever reaches a chef's screen or
// an exported file. Sesame is enforced with the exact same "never present, full stop" severity as
// nuts -- NOT merely flagged as an allergen (suggest-dish-ingredients' own ALLERGEN_VOCAB no
// longer lists it, for the same reason nuts were never in that list either).
//
...
const NUT_TERMS = [
  ...
  // -- sesame and sesame-derived products (same "never present" severity as nuts, added by
  // explicit school policy) -- catches "sesame seeds", "sesame oil", "black sesame", etc. via the
  // single bare "sesame" entry, same "whole segment matched, not just the word" removal logic
  // filterNutIngredients already applies to every other term above --
  { label: 'sesame', re: /\bsesame\b/i },
  { label: 'tahini', re: /\btahinis?\b/i },
  { label: 'benne', re: /\bbennes?\b/i },
  { label: 'simsim', re: /\bsimsim\b/i },
  { label: 'gomashio', re: /\bgom[ae]shio\b|\bgomasio\b/i },
  // Halva and za'atar are ambiguous the same way nougat/satay/amaretto above are -- a halva CAN
  // be made from a semolina base instead of sesame, and some za'atar blends omit sesame seeds --
  // but the AI can't verify which variant it meant, so these block by default on the same
  // "false positive is far cheaper than a false negative" reasoning.
  { label: 'halva', re: /\bhalvahs?\b|\bhalvas?\b/i },
  { label: "za'atar", re: /\bza'?atar\b/i },
];
...
//
// Also reused as-is (main.js) on the same function's `allergens` string (e.g. "gluten - dairy" or
// a stray "nut" the model wrote despite ALLERGEN_RULE forbidding it) -- the segment-matching logic
// here has no idea whether it's scanning an ingredient phrase or a bare allergen word, and doesn't
// need to: a dash-separated string is a dash-separated string, and "no nuts, ever" is the same
// policy regardless of which column it's enforced against.
...
// Row-level counterpart to filterNutIngredients above, for callers that don't have a
// dash-separated string to split -- Recipe Generator's generated_recipe_ingredients rows are
// already structured ({name, quantity, unit, method}), so there's nothing to split, just one
// `name` string per row to test against the same NUT_TERMS blocklist. Returns the matched
// label(s), or an empty array when clean -- same "which term(s) matched" detail
// filterNutIngredients returns per removed segment, just for one string at a time.
function matchNutTerms(text) {
  if (!text) return [];
  return NUT_TERMS.filter((t) => t.re.test(text)).map((t) => t.label);
}

// AI Menu Generator only: the one term allowed in a dish NAME (confirmed with the school
// 2026-09-23). This kitchen makes za'atar sesame-free as standard practice (thyme / sumac /
// oregano), the same way it makes pesto without pine nuts and hummus without tahini, so
// "Za'atar Manakish" is a legitimate menu item. Safety for it comes from the INGREDIENTS, which
// are still checked with the full list via matchNutTerms -- a key ingredient that says
// "za'atar" (or "sesame") still blocks the dish. Every other feature is unchanged.
const DISH_NAME_ALLOWED_LABELS = new Set(["za'atar"]);

function matchNutTermsInDishName(name) {
  return matchNutTerms(name).filter((label) => !DISH_NAME_ALLOWED_LABELS.has(label));
}

// Strips a matched nut/sesame WORD out of a longer piece of free text, rather than dropping the
// whole thing (filterNutIngredients's per-segment behavior) or just flagging it (matchNutTerms).
// Built specifically for the AI photo feature's dish-NAME field (generateDishImage/
// generate-recipe-photo): a dish literally named "Walnut Baklava" or "Sesame Chicken" still leaks
// the banned word into the image prompt's own title text even after its INGREDIENT list is fully
// filtered -- confirmed as a real, not theoretical, failure mode: a test generation for "Walnut
// Baklava" with "walnuts" correctly removed from the ingredient list still came back with a
// visibly nut-textured filling, because the model's general knowledge of what "baklava" (and the
// word "walnut" in the title) looks like overrode the ingredient list. Every NUT_TERMS regex is
// reused with a 'g' flag added so every occurrence is removed, not just the first.
function stripNutTermsFromText(text) {
  if (!text) return text || '';
  let result = text;
  for (const term of NUT_TERMS) {
    const flags = term.re.flags.includes('g') ? term.re.flags : `${term.re.flags}g`;
    result = result.replace(new RegExp(term.re.source, flags), '');
  }
  return result.replace(/\s+/g, ' ').trim();
}

module.exports = { filterNutIngredients, matchNutTerms, matchNutTermsInDishName, stripNutTermsFromText };
