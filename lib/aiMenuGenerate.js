...
const { SECTION_SLOTS, schoolDaysFrom, schoolDayCountBetween } = require('./generator');
...
// after that, so a pool never needs more than 20 days' worth of unique dishes.
...
const GUIDANCE = {
  ...
  LUNCH_SALAD: 'The vegetable side for Daycare lunch: a simple, soft salad or soft cooked vegetables suitable for toddlers. No meat.',
  ...
  PM_SNACK: 'The light afternoon snack before home time: fruit-based desserts, puddings, small baked goods, crackers with a dip, yogurt cups and similar.',
...
function poolSpecs() {
  const perSection = (sectionCode, category, perDay, extra = {}) => ({
    ...
    audience: AUDIENCE[sectionCode], ruleAttrs: [], spreadAttr: null,
    initialGroups: (d) => [{ count: withMargin(d * perDay) }], ...extra,
  });
  ...
  const coupled = (category, extra) => ({
    ...
    audience: AUDIENCE.KG_LP_MS_UP, ...extra,
  });
  return [
    amSnack('DAYCARE'),
    ...
    // 2 KG-LP/MS-UP mains - 1 Daycare main. Each needs one vegetarian of its own every day (the
    // engine's composition rule doesn't count a shared dish).
    //
    ...
    perSection('STAFF', 'STAFF_MAIN', 4, {
      ruleAttrs: ['protein_code'], spreadAttr: 'protein_code', compositionTopupFactor: 2,
      initialGroups: (d) => [
        { count: withMargin(Math.ceil(2.2 * d)), fixed: { protein_code: 'VEGETARIAN' } },
        { count: withMargin(3 * d), spread: { attr: 'protein_code', values: MEAT_PROTEINS } },
      ],
    }),
...
function normalizeAiDish(raw, pool, proteinCodes) {
  ...
  for (const attr of REQUIRED_ATTRS[pool.category] || []) {
...
class DraftRun {
  ...
  accept(raw, pool) {
    ...
    const safety = checkDishSafety({ ...dish, section_codes: pool.sections });
    if (!safety.ok) {
  ...
  async _generateForPool(pool, groups) {
    ...
    for (const callGroups of planCalls(groups)) {
      this.stats.aiCalls++;
      const request = {
        category: { code: pool.category, label: category.name || pool.category, guidance: GUIDANCE[pool.category] },
        audience: pool.audience,
        seafoodAllowed: !pool.sections.some(isStudentSection),
        proteinCodes: this.proteinCodes.filter((c) => pool.sections.some(isStudentSection) ? c !== 'FISH' : true),
        requiredAttrs: REQUIRED_ATTRS[pool.category] || [],
        groups: callGroups.map((g) => ({
          count: g.count,
          ...(g.fixed ? { fixed: g.fixed } : {}),
          ...(g.spread ? { spread: { attr: g.spread.attr, ...(g.spread.values ? { values: g.spread.values.filter((v) => g.spread.attr !== 'protein_code' || this.proteinCodes.includes(v)) } : {}) } } : {}),
        })),
        avoidNames: this._avoidNames(pool),
        styleExamples,
      };
      let dishes;
      try {
      ...
      for (const raw of dishes) this.accept(raw, pool);
    }
  }
  ...
  analyzeShortfalls(out) {
    ...
    const need = (pool, sig, n = 1) => {
      if (!needs.has(pool.key)) needs.set(pool.key, new Map());
      const m = needs.get(pool.key);
    ...
    for (const [sectionCode, { resultDays, warnings }] of Object.entries(out)) {
        ...
        if ((m = REUSE.exec(w))) {
          ...
          if (pool) {
            ...
            if (protein) {
              ...
              need(pool, sig, (Number(m[4]) || 1) * (pool.compositionTopupFactor || 1));
              continue;
            }
          }
        } else if ((m = STYLE.exec(w))) {
          const pool = this.poolBySectionCategory.get(`${sectionCode}:AM_SNACK`);
          if (pool) { need(pool, { am_snack_style: m[3] }); continue; }
        }
      }
      ...
      for (const [category, , options] of SECTION_SLOTS[sectionCode]) {
        ...
        for (const day of resultDays) {
          ...
          if (sectionCode === 'STAFF' && category === 'STAFF_BREAKFAST') {
            for (const s of ['DAYCARE', 'KG_LP', 'MS_UP']) {
              const d = out[s] && out[s].resultDays.find((x) => x.date === day.date);
              for (const it of (d ? d.items.filter((i) => i.category === 'AM_SNACK') : [])) forcedIds.add(it.id);
            }
          }
    ...
    for (const [key, sigs] of needs) {
      ...
      const cap = Math.min(this.numWeekdays, NO_REPEAT_SCHOOL_DAYS) * pool.perDay;
      let total = 0;
      const groups = [];
      for (const [sigJson, n] of sigs) {
        ...
        groups.push({ count, ...(Object.keys(fixed).length ? { fixed } : {}), ...(spreadAttr ? { spread: { attr: spreadAttr } } : {}) });
      }
      requests.set(key, groups);
    }
    ...
    for (const day of out[section].resultDays) {
      ...
      for (const it of day.items) {
        ...
        if (section === 'MS_UP' && ['LUNCH_MAIN', 'LUNCH_STARCH'].includes(it.category)) {
          ...
          if (idsIn('KG_LP', day.date, 'LUNCH_MAIN').has(it.id)) source = 'KG_LP';
          else if (idsIn('DAYCARE', day.date, 'LUNCH_MAIN').has(it.id)) source = 'DAYCARE';
...
async function generateDraftRun({ label, startDate, endDate, createdBy, onProgress }) {
  ...
  try {
    await run.load();
    const d = Math.min(numWeekdays, NO_REPEAT_SCHOOL_DAYS);
    await run.generatePools(new Map(run.pools.map((p) => [p.key, p.initialGroups(d)])), 'Inventing dishes');
...
module.exports = { generateDraftRun, poolSpecs, planCalls, DraftRun, buildPickRows, normalizeAiDish };
