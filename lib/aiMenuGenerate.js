...
const { SECTION_SLOTS, SNACK_STYLE_BY_PATTERN, schoolDaysFrom, schoolDayCountBetween } = require('./generator');
...
const AUDIENCE = {
  ...
  // Daycare and KG-LP share one AM Snack and one PM Snack a day (2026-09-24): Daycare's limits apply.
  DAYCARE_KG_LP: 'Children from Daycare to lower primary, about 1 to 8 years old -- the SAME snack is served to all of them, so it must suit the youngest: soft, mild, easy to chew, cut small; no chili heat, nothing hard, round or chokeable (whole grapes, whole cherry tomatoes, hard raw vegetables, popcorn, hard crackers or pretzels).',
...
const GUIDANCE = {
  AM_SNACK: 'The mid-morning snack, one item per child. Either PASTRY style (baked or griddled: muffins, pancakes, croissants, manakish, waffles, cake slices, cheese rolls) or COLD_KITCHEN style (assembled savory: sandwiches, wraps, mini pizzas, omelettes, cheese or vegetable plates). NEVER chicken or beef in any form -- those are served at lunch only; turkey, egg, cheese, labneh and vegetarian fillings are fine.',
  ...
  PM_SNACK: 'The light afternoon snack before home time, either PASTRY style (am_snack_style PASTRY: vanilla cake, muffins, date balls, small sweet baked goods) or the savory / salty side (am_snack_style COLD_KITCHEN: salted pretzels, baked vegetable chips, crackers, savory bites, cheese fingers). Never manakish of any kind -- manakish is served only at the morning snack. NEVER chicken or beef in any form -- those are served at lunch only.',
  STAFF_BREAKFAST: 'One dish on the staff breakfast buffet (six a day): egg dishes, pastries, sandwiches and wraps, oats / granola / yogurt bowls, cheese plates, foul, shakshuka and other hot or cold breakfast items. am_snack_style PASTRY = baked / griddled sweet or bread-based items (croissants, manakish, muffins, pancakes, cakes, pastries); COLD_KITCHEN = assembled or plated savory items (sandwiches, wraps, egg dishes, foul, labneh / cheese / vegetable plates).',
...
const REQUIRED_ATTRS = {
  ...
  // The Pastry / Cold Kitchen style drives the daily snack rotation and Staff Breakfast's 3 + 3 mix.
  PM_SNACK: ['am_snack_style'],
  STAFF_BREAKFAST: ['dish_concept', 'am_snack_style'],
...
// Turkey counts as meat (TURKEY protein type, 2026-09-24); request spreads drop codes the live
// protein_types table doesn't have (buildDishRequest).
const MEAT_PROTEINS = ['CHICKEN', 'BEEF', 'LAMB', 'FISH', 'TURKEY'];
...
function poolSpecs() {
  ...
  // in that cuisine, plus a spare, so every rule can hold on the day (ctx.style = that day's snack
  // styles, SNACK_STYLE_BY_PATTERN's { AM_SNACK: { section: style }, PM_SNACK: ... }). Default: the
  // slot's count + 1.
  ...
  // A snack cell alternates Pastry / Cold Kitchen daily, so half its days need each style. `sections`
  // is ['DAYCARE', 'KG_LP'] for the shared Daycare / KG-LP snacks, whose style follows Daycare's.
  const snack = (sections, category, extra = {}) => {
    const spread = category === 'AM_SNACK' ? { spread: { attr: 'dish_concept' } } : {};
    return {
      ...perSection(sections[0], category, 1),
      key: `${sections.join('+')}:${category}`, sections,
      audience: sections.length > 1 ? AUDIENCE.DAYCARE_KG_LP : AUDIENCE[sections[0]],
      ruleAttrs: ['am_snack_style'], spreadAttr: category === 'AM_SNACK' ? 'dish_concept' : null,
      initialGroups: (d) => ['PASTRY', 'COLD_KITCHEN'].map((style) => ({
        count: withMargin(Math.ceil(d / 2)), fixed: { am_snack_style: style }, ...spread,
      })),
      tuesdayGroups: (ctx) => [{ count: 2, fixed: { am_snack_style: ctx.style[category][sections[0]] }, ...spread }],
      ...extra,
    };
  };
  ...
  return [
    // Daycare and KG-LP share one AM Snack and one PM Snack a day; MS-UP has its own (2026-09-24).
    snack(['DAYCARE', 'KG_LP'], 'AM_SNACK'),
    snack(['DAYCARE', 'KG_LP'], 'PM_SNACK', {
      guidance: 'The afternoon snack shared by Daycare and KG-LP (ages 1 to 8): SOFT items only, nothing hard or crunchy (choking risk). PASTRY style (am_snack_style PASTRY): vanilla cake, soft muffins, date balls, soft sweet bakes. The savory side (am_snack_style COLD_KITCHEN) must be soft too: labneh roll-ups, soft cheese fingers, soft savory pinwheels -- no hard crackers, hard pretzels or chips. Never manakish -- that is only for the morning snack. Never chicken or beef (lunch only).',
    }),
    ...
    // Staff's own picks per day, after the shared ones: Breakfast 6 - 2 school AM Snacks (the shared
    // Daycare / KG-LP one + MS-UP's) = 4, always 2 Pastry + 2 Cold Kitchen; Main 7 - 5 shared (KG-LP/
    // MS-UP 2 mains + 2 starches, MS-UP's vegetable) = exactly 1 vegan + 1 vegetarian.
    //
    ...
    // Six dishes and six dish types means "different types" is one of each type every day, so Staff's
    // own four are exactly the four types the two shared AM Snacks leave, AND 2 Pastry + 2 Cold
    // Kitchen: few dishes fit a given day. Every group is split by style, the general ones are twice
    // one-a-day (smaller pools left repeats at 21-27 days in testing), and the meat-free dish can be
    // either style, so half of those are each.
    perSection('STAFF', 'STAFF_BREAKFAST', 4, {
      // protein_code too: a repeated meat-free dish must be topped up with a meat-free one.
      ruleAttrs: ['dish_concept', 'am_snack_style', 'protein_code'], spreadAttr: 'dish_concept', compositionTopupFactor: 2,
      initialGroups: (d) => ['PASTRY', 'COLD_KITCHEN'].flatMap((style) => [
        { count: withMargin(d), fixed: { protein_code: 'VEGETARIAN', am_snack_style: style }, spread: { attr: 'dish_concept' } },
        { count: withMargin(2 * d), fixed: { am_snack_style: style }, spread: { attr: 'dish_concept' } },
      ]),
      // The 2 shared AM Snacks' types aren't known in advance, so one dish of every type in each style,
      // plus meat-free ones of each style: any 2 + 2 free types can then be filled, one meat-free.
      tuesdayGroups: () => ['PASTRY', 'COLD_KITCHEN'].flatMap((style) => [
        { count: ENUMS.dish_concept.length, fixed: { am_snack_style: style }, spread: { attr: 'dish_concept', values: ENUMS.dish_concept } },
        // Three meat-free per style: with one, both could share a type with the day's shared snacks.
        { count: 3, fixed: { protein_code: 'VEGETARIAN', am_snack_style: style }, spread: { attr: 'dish_concept', values: ENUMS.dish_concept } },
      ]),
    }),
...
class DraftRun {
  ...
  analyzeShortfalls(out) {
    ...
    const STYLE = /^(\w+) (\S+): no (PASTRY|COLD_KITCHEN) (AM|PM) Snack items available/;
    const STYLE_MIX = /^(\w+) (\S+): (\w+) style mix short of (PASTRY|COLD_KITCHEN)/;
    ...
    for (const [sectionCode, { resultDays, warnings }] of Object.entries(out)) {
      ...
      for (const w of warnings) {
        ...
        if ((m = REUSE.exec(w))) {
          ...
          const pool = this.poolBySectionCategory.get(`${sectionCode}:${m[4]}_SNACK`);
          if (pool) { need(pool, { am_snack_style: m[3] }, 1, m[2]); continue; }
        } else if ((m = STYLE_MIX.exec(w))) {
          // Staff Breakfast's 3 + 3 mix gave way: order a dish of the missing style.
          const pool = this.poolBySectionCategory.get(`${sectionCode}:${m[3]}`);
          if (pool) { need(pool, { am_snack_style: m[4] }, 1, m[2]); continue; }
  ...
  for (const section of DRAFT_SECTION_ORDER) {
    for (const day of out[section].resultDays) {
      ...
      for (const it of day.items) {
        ...
        if (section === 'MS_UP' && ['LUNCH_MAIN', 'LUNCH_STARCH'].includes(it.category)) {
        ...
        } else if (section === 'KG_LP' && ['AM_SNACK', 'PM_SNACK'].includes(it.category)) {
          // KG-LP's snacks are Daycare's (shared, 2026-09-24).
          if (idsIn('DAYCARE', day.date, it.category).has(it.id)) source = 'DAYCARE';
          ...
          // The shared Daycare / KG-LP AM Snack is Daycare's (its source); MS-UP's is its own.
          source = ['DAYCARE', 'MS_UP', 'KG_LP'].find((s) => idsIn(s, day.date, 'AM_SNACK').has(it.id)) || null;
        }
...
async function generateDraftRun({ label, startDate, endDate, createdBy, onProgress }) {
  ...
  try {
    ...
    for (const t of tuesdays) {
      const style = SNACK_STYLE_BY_PATTERN[t.index % 2 === 1 ? 'A' : 'B'];
