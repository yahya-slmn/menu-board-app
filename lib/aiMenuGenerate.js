...
const GUIDANCE = {
  ...
  PM_SNACK: 'The light afternoon snack before home time, either PASTRY style (vanilla cake, muffins, date balls, small baked goods) or SALTY-SNACK style (salted pretzels, baked vegetable chips, crackers, savory bites). Never manakish of any kind -- manakish is served only at the morning snack. NEVER chicken or beef in any form -- those are served at lunch only.',
...
function poolSpecs() {
  ...
  return [
    ...
    perSection('DAYCARE', 'PM_SNACK', 1, {
      guidance: 'The Daycare afternoon snack: SOFT pastry-style or savory items only (vanilla cake, soft muffins, date balls, soft pretzel bites) -- nothing hard or crunchy, no hard pretzels or chips (choking risk). Never manakish -- that is only for the morning snack. Never chicken or beef (lunch only).',
    }),
    amSnack('KG_LP'),
