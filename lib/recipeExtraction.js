...
// `targetLanguage` is the chef's pick from the Upload Recipe language selector -- forwarded
// as-is; the Edge Function defaults to English if it's missing or blank.
async function extractRecipeFromFile({ files, targetLanguage }) {
  const { data, error } = await supabase.functions.invoke('extract-recipe', {
    body: { files, targetLanguage },
  });
