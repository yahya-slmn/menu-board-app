...
async function loadReferenceData() {
  const [
    ...
    { data: portionDefaults, error: pdErr },
  ] = await Promise.all([
    ...
    supabase.from('category_portion_defaults').select('*'),
  ]);
  ...
  if (pdErr) throw supaFail('loadReferenceData: category_portion_defaults', pdErr);

  const portionDefaultsByKey = new Map(
    portionDefaults.map(pd => [`${pd.category_id}:${pd.section_id}`, { unit: pd.unit, quantity: pd.quantity }])
  );
  ...
  cache = {
    sections, categories: categoriesEnriched, proteinTypes, ageGroups, mealPeriods,
    portionDefaultsByKey,
  };
}
...
// --- category portion defaults (one per category+section, e.g. Lunch Main Course = 140g for
// Daycare but 120g for KG-LP/MS-UP) -- quantity is stored as text since some rows are
// legitimately non-numeric ("05 option" for MS-UP's Fruit Basket). Returns null when no
// default has been set for that (category, section) pair yet.
function getCategoryPortionDefault(categoryId, sectionId) {
  return data().portionDefaultsByKey.get(`${categoryId}:${sectionId}`) || null;
}
