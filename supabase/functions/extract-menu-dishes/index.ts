...
2. When a row has NO specific sub-category cell at all (only a broad period label like "AM Snack" applies, exactly the same label a genuine snack dish elsewhere in that block also carries) but the dish name itself is unambiguously just a plain milk, juice, water, whole/mixed-fruit, or plain bread/bread-basket item with no preparation involved (e.g. "Plain Fresh Milk Full Fat", "Fresh Full Fat Milk", "lemonade Juice OR Laban", "Fruit (a selection of seasonal fruits)", "Pita Bread", "Dinner Roll", "Whole Wheat Bread", "Arabic Bread"), output the item's own specific type instead of the surrounding period label -- "Milk" for a plain milk item, "Juice" for a juice/yogurt-drink choice, "Fruit Basket" for a whole/mixed fruit selection, "Bread" for a plain bread/roll/bun item with no filling or preparation -- even though that exact word never appears in the sheet for that row. This distinction matters downstream: a broad period label like "AM Snack"/"Lunch" can include both a real dish AND a plain drink/fruit/bread item side by side, and only the specific type lets them be told apart. A composed bread-based DISH (e.g. "Garlic Bread", "Cheese Manakish", a sandwich) is not a plain bread item -- give it its own specific category if one is visible, or leave the broad period label if not; don't recategorize it as "Bread".
